...
public class ConversationManagerKit implements TIMRefreshListener, MessageRevokedManager.MessageRevokeHandler {
    ...
    public void loadConversation(final IUIKitCallBack callBack) {
        ...
        TIMGroupManager.getInstance().getGroupList(new TIMValueCallBack<List<TIMGroupBaseInfo>>() {
            ...
            public void onError(int code, String desc) {
                loadConversation(null, callBack);
            }

            @Override
            public void onSuccess(List<TIMGroupBaseInfo> timGroupBaseInfos) {
                loadConversation(timGroupBaseInfos, callBack);
     ...
     * 加载会话信息
     *
     * @param timGroupBaseInfos
     * @param callBack
     */
    private void loadConversation(List<TIMGroupBaseInfo> timGroupBaseInfos, IUIKitCallBack callBack) {
        ...
        for (int i = 0; i < timConversations.size(); i++) {
            ...
            final ConversationInfo conversationInfo = TIMConversation2ConversationInfo(conversation);
                ...
                if (conversationInfo.isGroup()) {
                    ...
                    if (null != timGroupBaseInfos && !timGroupBaseInfos.isEmpty()) {
                        for (TIMGroupBaseInfo timGroupBaseInfo : timGroupBaseInfos) {
                            if (TextUtils.equals(conversationInfo.getId(), timGroupBaseInfo.getGroupId())) {
                                isHas = true;
                                break;
                            }
                        }
                    ...
                    if (isHas) {
                        ...
                        TIMManager.getInstance().deleteConversationAndLocalMsgs(TIMConversationType.Group, conversationInfo.getId());
                    }
                } else {
                    mUnreadTotal = mUnreadTotal + conversationInfo.getUnRead();
                    infos.add(conversationInfo);
    ...
    public void onRefreshConversation(List<TIMConversation> conversations) {
        TUIKitLog.e("onRefreshConversation");
        if (mProvider == null) {
            return;
        }
        ArrayList<ConversationInfo> infos = new ArrayList<>();
        for (int i = 0; i < conversations.size(); i++) {
            ...
            TUIKitLog.e("onRefreshConversation TIMConversation " + conversation.toString());
        ...
        for (int j = 0; j < infos.size(); j++) {
            ...
            for (int i = 0; i < dataSource.size(); i++) {
                ...
                if (cacheInfo.getId().equals(update.getId())) {
                    ...
                    TUIKitLog.e("onRefreshConversation after mUnreadTotal = " + mUnreadTotal);
                    exist = true;
                    break;
                }
            }
            if (!exist) {
                ...
                TUIKitLog.e("onRefreshConversation exist = " + exist + ", mUnreadTotal = " + mUnreadTotal);
            }
        }
    ...
    private ConversationInfo TIMConversation2ConversationInfo(TIMConversation conversation) {
        ...
        TUIKitLog.e("onRefreshConversation ext.getUnreadMessageNum() " + conversation.getUnreadMessageNum());
        return info;
    }
    ...
    public void deleteConversation(int index, ConversationInfo conversation) {
        boolean status = TIMManager.getInstance().deleteConversationAndLocalMsgs(conversation.isGroup() ? TIMConversationType.Group : TIMConversationType.C2C, conversation.getId());
        if (status) {
    ...
    public void deleteConversation(String id, boolean isGroup) {
        TUIKitLog.e("deleteConversation id = " + id);
        handleTopData(id, false);
        List<ConversationInfo> conversationInfos = mProvider.getDataSource();
        for (int i = 0; i < conversationInfos.size(); i++) {
            ConversationInfo info = conversationInfos.get(i);
            if (info.getId().equals(id)) {
                mUnreadTotal = mUnreadTotal - info.getUnRead();
                break;
            }
        }
        if (mProvider != null) {
