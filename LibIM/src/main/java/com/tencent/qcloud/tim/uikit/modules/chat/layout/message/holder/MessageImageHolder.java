...
import com.tencent.qcloud.tim.uikit.component.IMMessagePreviewActivity;
...
//                TUIKit.getAppContext().startActivity(intent);
                try {
                    /*图片点击跳转*/
                    Intent previewIntent = new Intent(TUIKit.getAppContext(), IMMessagePreviewActivity.class);
                    previewIntent.putExtra(IMMessagePreviewActivity.KEY_CONVERSTION_TYPE, msg.isGroup() ? 1 : 0);
                    previewIntent.putExtra(IMMessagePreviewActivity.KEY_CONVERSTION_ID, msg.getTIMMessage().getConversation().getPeer());
                    previewIntent.putExtra(IMMessagePreviewActivity.KEY_MESSAGE_ID, msg.getId());
                    previewIntent.setFlags(FLAG_ACTIVITY_NEW_TASK);
                    TUIKit.getAppContext().startActivity(previewIntent);
                } catch (Exception e) {
                    TUIKitLog.e("图片点击跳转预览=========" + e.getMessage());
                }
    ...
    private void getVideo(TIMVideo video, String videoPath, final MessageInfo msg, final boolean autoPlay, final int position) {
        video.getVideo(videoPath, new TIMCallBack() {
            @Override
            public void onError(int code, String desc) {
                TUIKitLog.e(String.format(Locale.CHINA, "下载视频失败    errCode：%s  errMsg：%s", code, desc));
...
//        try {
//            String videoPath = ((TIMVideoElem) msg.getTIMMessage().getElement(0)).getVideoPath();
//            intent.putExtra(TUIKitConstants.VIDEO_PATH, videoPath);
//        } catch (Exception e) {
//            e.printStackTrace();
//        }
...
//        TUIKit.getAppContext().startActivity(intent);
        try {
            ...
            TUIKitLog.e("跳转视频预览=========" + e.getMessage());
        }
    }
