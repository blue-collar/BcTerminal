...
public abstract class MessageContentHolder extends MessageEmptyHolder {
    ...
    public void layoutViews(final MessageInfo msg, final int position) {
        ...
        if (msg.isSelf()) { // 默认不显示自己的昵称
            tv_orgName.setVisibility(View.GONE);
            ...
            if (properties.getLeftNameVisibility() == 0) {
                if (msg.isGroup()) { // 群聊默认显示对方的昵称
                    ...
                    tv_orgName.setVisibility(View.VISIBLE);
                } else { // 单聊默认不显示对方昵称
                    usernameText.setVisibility(View.GONE);
                    tv_orgName.setVisibility(View.GONE);
                ...
                tv_orgName.setVisibility(properties.getLeftNameVisibility());
            }
            if (usernameText.getVisibility() == View.GONE) {
                RelativeLayout.LayoutParams params = (RelativeLayout.LayoutParams) msgContentLinear.getLayoutParams();
                params.topMargin = 17;
        ...
        if (properties.getNameFontColor() != 0) {
            ...
            tv_orgName.setTextColor(properties.getNameFontColor());
        }
        ...
        if (profile == null) {
            ...
            if (null != customInfo.get("ExtData")) {
                IMUserExtObj extData = IMJsonUtil.parseJsonToBean(new String(customInfo.get("ExtData")), IMUserExtObj.class);
                tv_orgName.setText(null == extData || null == extData.organName ? "" : TextUtils.concat("(", extData.organName, ")"));
            } else {
                tv_orgName.setText("");
            }
        }
        ...
        if (msg.isSelf()) {
            ...
            tv_time.setText(DateTimeUtil.getTimeFormatText(new Date(msg.getTIMMessage().timestamp() * 1000)));
        }
