...
public class MessageInfoUtil {
     ...
     * @return
     */
    public static MessageInfo buildLocationMessage(String address, double latitude, double longitude) {
     ...
     * @return
     */
    public static MessageInfo buildCustomTRTCMessage(String desc, String data) {
        MessageInfo info = new MessageInfo();
        TIMMessage TIMMsg = new TIMMessage();
        TIMCustomElem ele = new TIMCustomElem();
        ele.setData(data.getBytes());
        if (AUDIO_CALL_MESSAGE_DESC.equals(desc)) {
            ele.setDesc("[语音通话]");
        } else if (VIDEO_CALL_MESSAGE_DESC.equals(desc)) {
            ele.setDesc("[视频通话]");
        }
        TIMMsg.addElement(ele);
        info.setSelf(true);
     ...
     * @return
     */
    public static MessageInfo TIMMessage2MessageInfo(final TIMMessage timMessage, boolean isGroup) {
...
//                    String desc = new String(customElem.getDesc());
//                    if (AUDIO_CALL_MESSAGE_DESC.equals(desc)) {
//                        msgInfo.setExtra("[语音通话]");
//                    }else  if (VIDEO_CALL_MESSAGE_DESC.equals(desc)) {
...
//                    msgInfo.setMsgType(MessageInfo.MSG_TYPE_CUSTOM);
                ...
                if (tipsType == TIMGroupTipsType.ModifyGroupInfo) {
                    ...
                    if (modifyList.size() > 0) {
                        ...
                        if (modifyType == TIMGroupTipsGroupInfoType.ModifyName) {
                            ...
                            message = message + "修改讨论组名称为\"" + modifyInfo.getContent() + "\"";
     ...
     * @param callBack
     */
    public static void getUsersProfile(Map<String, TIMGroupMemberInfo> memberDetails, TIMValueCallBack<List<TIMUserProfile>> callBack) {
        if (null != memberDetails && !memberDetails.isEmpty()) {
            /*待获取用户资料的用户列表*/
            List<String> users = new ArrayList<>();
            for (Map.Entry<String, TIMGroupMemberInfo> entry : memberDetails.entrySet()) {
                users.add(entry.getValue().getUser());
            }
            /*获取用户资料*/
            TIMFriendshipManager.getInstance().getUsersProfile(users, false, callBack);
        }
    }
     ...
     * @param callBack
     */
    public static void getUsersProfile(String userId, TIMValueCallBack<List<TIMUserProfile>> callBack) {
     ...
     * @param callBack
     */
    public static void getUsersProfile(List<TIMGroupMemberInfo> memberDetails, TIMValueCallBack<List<TIMUserProfile>> callBack) {
        if (null != memberDetails && !memberDetails.isEmpty()) {
            /*待获取用户资料的用户列表*/
            List<String> users = new ArrayList<>();
            for (TIMGroupMemberInfo memberDetail : memberDetails) {
