...
public class GroupChatManagerKit extends ChatManagerKit {
     ...
     * @param callBack
     */
    public void createGroupChat(final GroupInfo chatInfo, final String owerName, final IUIKitCallBack callBack) {
        ...
        param.setNotification(chatInfo.getNotice());
        List<TIMGroupMemberInfo> infos = new ArrayList<>();
        if (null != chatInfo.getMemberDetails()) {
        ...
        TIMGroupManager.getInstance().createGroup(param, new TIMValueCallBack<String>() {
            ...
            public void onSuccess(final String groupId) {
                ...
                String message = TextUtils.concat("'", owerName, "' 创建了讨论组").toString();
     ...
     * @param callBack
     */
    public void applyJoinGroup(final String groupId, final String owerName, final IUIKitCallBack callBack) {
        TIMGroupManager.getInstance().applyJoinGroup(groupId, "警情添加", new TIMCallBack() {
            @Override
            ...
            public void onSuccess() {
                String message = TextUtils.concat("'", owerName, "' 加入了讨论组").toString();
                final MessageInfo createTips = MessageInfoUtil.buildGroupCustomMessage(MessageInfoUtil.GROUP_CREATE, message);
                TIMConversation conversation = TIMManager.getInstance().getConversation(TIMConversationType.Group, groupId);
                try {
                    Thread.sleep(200);
     ...
     * @param callBack
     */
    public void sendNoticeMessage(final String groupId, String message, final IUIKitCallBack callBack) {
        if (!TextUtils.isEmpty(message)) {
            TIMConversation conversation = TIMManager.getInstance().getConversation(TIMConversationType.Group, groupId);
            MessageInfo createTips = MessageInfoUtil.buildGroupCustomMessage(MessageInfoUtil.GROUP_CREATE, message);
            sendTipsMessage(conversation, createTips.getTIMMessage(), new IUIKitCallBack() {
                @Override
                public void onSuccess(Object data) {
...
//            TUIKitLog.e("您所在的群" + groupSysEle.getGroupId() + "已解散");
            if (mCurrentChatInfo != null && groupSysEle.getGroupId().equals(mCurrentChatInfo.getId())) {
                onGroupForceExit();
            }
        }
