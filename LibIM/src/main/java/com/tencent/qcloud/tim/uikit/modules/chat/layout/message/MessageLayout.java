...
public class MessageLayout extends MessageLayoutUI {
    ...
    private void initPopActions(final MessageInfo msg) {
        ...
        actions.add(action);
        if (msg.isSelf()) {
            if ((System.currentTimeMillis() - msg.getMsgTime()) < (1.5 * 60 * 1000) && msg.getStatus() == MessageInfo.MSG_STATUS_SEND_SUCCESS) {
                action = new PopMenuAction();
            ...
            if (msg.getStatus() == MessageInfo.MSG_STATUS_SEND_SUCCESS) {
                initForwardingAction(actions, msg);
            }
        } else {
            initForwardingAction(actions, msg);
        }
     ...
     * @param msg
     */
    private void initForwardingAction(List<PopMenuAction> actions, final MessageInfo msg) {
        if (msg.getMsgType() == MessageInfo.MSG_TYPE_TEXT ||
                msg.getMsgType() == MessageInfo.MSG_TYPE_IMAGE ||
                msg.getMsgType() == MessageInfo.MSG_TYPE_VIDEO ||
                msg.getMsgType() == MessageInfo.MSG_TYPE_FILE ||
                msg.getMsgType() == MessageInfo.MSG_TYPE_LOCATION) {
            PopMenuAction action = new PopMenuAction();
            action.setActionName("转发");
            action.setActionClickListener(new PopActionClickListener() {
                @Override
                public void onActionClick(int position, Object data) {
                    mOnItemClickListener.onForwardingMessageClick(msg);
                }
            });
            actions.add(action);
        }
    ...
    public void postSetAdapter(MessageListAdapter adapter) {
        mAdapter.setOnItemClickListener(new MessageLayout.OnItemClickListener() {
            ...
            public void onMessageClick(View view, MessageInfo messageInfo) {
                mOnItemClickListener.onMessageClick(view, messageInfo);

            }

            @Override
            public void onForwardingMessageClick(MessageInfo messageInfo) {
                mOnItemClickListener.onForwardingMessageClick(messageInfo);
            }
        });
    }
    ...
    public interface OnItemClickListener {
        ...
        void onMessageClick(View view, MessageInfo messageInfo);

        void onForwardingMessageClick(MessageInfo messageInfo);
    }
