package com.telewave.battlecommand.directboard;
...
import com.telewave.battlecommand.trtc.TRTCVideoView;
 ...
 */
public class TRTCLiveBoardViewLayout extends RelativeLayout {
    private final static String TAG = TRTCLiveBoardViewLayout.class.getSimpleName();
    // 前后堆叠模式
    ...
    public interface ITRTCVideoViewLayoutListener {
         ...
         * @param userName
         */
        void onChangeVideoShowFrame(String userId, String userName);
    }

    public TRTCLiveBoardViewLayout(Context context) {
        super(context);
        initView(context);
    }


    public TRTCLiveBoardViewLayout(Context context, AttributeSet attrs) {
        super(context, attrs);
        initView(context);
    }

    public TRTCLiveBoardViewLayout(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    public void initFloatLayoutParams() {
        ...
        int bottomMargin = dip2px(50);
        int subWidth = dip2px(90);
        int subHeight = dip2px(156);
...
//            }
    ...
    public ArrayList<TXCloudVideoView> getmVideoViewList() {
...
//            }
        ...
        view.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (object != null) {
                    ...
                    Log.e(TAG, "click on pos: " + pos + "/userId1: " + renderView.getUserId());
                    Log.e(TAG, "click on pos: " + pos + "/userId2: " + renderView.getGLSurfaceView());
                    Log.e(TAG, "click on pos: " + pos + "/userId3: " + renderView.getVideoView());
                     ...
                     * 更改大视频显示文字
                     */
                    String useNameTemp = getUserName(renderView.getUserId());
                    ITRTCVideoViewLayoutListener listener = mListener.get();
                    if (listener != null) {
                        String userId = renderView.getUserId();
                        if (userId.length() > 32) {
                            userId = userId.substring(0, userId.length() - 1);
                        }
                        listener.onChangeVideoShowFrame(userId, useNameTemp);
                    }
                }
            }
        });
    ...
    public void makeFullVideoView(int index) {// 1 -> 0
        Log.e(TAG, "makeFullVideoView: = " + index);
...
//        fullVideoView.setMoveable(true);
        ...
        // 将 fromView 塞到 0 的位置
        mVideoViewList.set(0, indexView);
        ...
        for (int i = 0; i < mVideoViewList.size(); i++) {
            ...
            final ITRTCVideoViewLayoutListener listener = mListener.get();
            if (userId != null && userId.length() > 0) {
                if (userId.length() > 32) {
                    userId = userId.substring(0, userId.length() - 1);
                }
                listener.onChangeVideoFillMode(userId, false);
            }
    ...
    public TXCloudVideoView onMemberEnter(String userId) {
        ...
        int posIdx = 0;
        // 我本地渲染的位置
        int localRenderPos = mVideoViewList.size();
        for (int i = 0; i < mVideoViewList.size(); i++) {
            TXCloudVideoView renderView = mVideoViewList.get(i);
            if (renderView != null) {
                String vUserId = renderView.getUserId();
...
//            initFloatLayout();
    ...
    private String getUserName(String userId) {
        String useName = null;
        if (userId.startsWith(mSelfUserId)) {
            useName = ConstData.username + "（自己）";
        } else {
            if (ConstData.receiveUserSet != null && ConstData.receiveUserSet.size() > 0) {
                for (SampleUser sampleUser : ConstData.receiveUserSet) {
                    if (userId.startsWith(sampleUser.getUserid())) {
                        useName = sampleUser.getName();
                    }
                }
            }
        }
    ...
    private void addToolbarLayout(final TXCloudVideoView videoView) {
        ...
        if (view == null) {
            view = LayoutInflater.from(mContext).inflate(R.layout.video_layout_toolbar, videoView);
            ...
            final ImageView videoCallToChat = (ImageView) view.findViewById(R.id.videocall_to_chat);
            /**
             * 直播时隐藏
             */
            videoCallToChat.setVisibility(View.GONE);
            videoCallToChat.setOnClickListener(new OnClickListener() {
                @Override
                public void onClick(View v) {
                    String userId = videoView.getUserId();
                    if (userId != null) {
                        ...
                        if (onVideoToChatClickListener != null) {
                            onVideoToChatClickListener.onVideoToChatClick(userId);
                        }
                    }
                }
            });
    ...
    private void showNoVideoLayout(TXCloudVideoView videoView, boolean bShow) {
        View view = videoView.findViewById(R.id.layout_no_video);
        LinearLayout noVideolayout = videoView.findViewById(R.id.show_no_video_layout);
        if (view != null) {
            view.setVisibility(bShow ? VISIBLE : GONE);
            if (bShow) {
...
//                    }
        ...
        for (TXCloudVideoView videoView : mVideoViewList) {
            if (videoView.getVisibility() == VISIBLE) {
                ...
                if (tempUserID != null && tempUserID.startsWith(userID)) {
                    ...
                    if (textView != null) {
                        textView.setText(getUserName(userID));
                    }
                    if (bHasVideo == false) {
                        View layoutToolbar = videoView.findViewById(R.id.layout_toolbar);
                        if (layoutToolbar != null) {
    ...
    public interface OnVideoToChatClickListener {
        void onVideoToChatClick(String userId);
    }

    private OnVideoToChatClickListener onVideoToChatClickListener;

    public void setOnVideoToChatListener(OnVideoToChatClickListener onVideoToChatClickListener) {
        this.onVideoToChatClickListener = onVideoToChatClickListener;
    }
}
