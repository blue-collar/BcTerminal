 ...
 * @date 2019/1/9
 */
public class PolicyGuiFanFuJianAdapter extends BaseAdapter {
    private Context mContext;
    private List<PolicyGuiFan.FileListBean> fileListBeanList;

    public PolicyGuiFanFuJianAdapter(Context mContext, List<PolicyGuiFan.FileListBean> fileListBeanList) {
        super();
    ...
    public Object getItem(int position) {
        return this.fileListBeanList.get(position);
    }
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        final PolicyGuiFan.FileListBean fileListBean = fileListBeanList.get(position);
        holder.fujianFileName.setText(fileListBean.getFilename());
        holder.fujianUpdateTime.setText(fileListBean.getUpdatetime());

        File file = new File(ConstData.POLICY_GUI_FAN_DIR + fileListBean.getFilename());
        if (file.exists()) {
    ...
    class OnClickTextListener implements View.OnClickListener {
        ...
        @Override
        public void onClick(View v) {
            File file = new File(ConstData.POLICY_GUI_FAN_DIR + fileName);
            if (file.exists()) {
            ...
            if (TextUtils.isEmpty(HTTP_URL)) {
                //从服务器下载预案文档
                downLoadFile(mContext, viewHolder, ConstData.urlManager.baseFilesURL + downloadUrl, ConstData.POLICY_GUI_FAN_DIR, fileName);
            } else {
                //从服务器下载预案文档
                downLoadFile(mContext, viewHolder, ConstData.urlManager.baseFilesURL + HTTP_URL, ConstData.POLICY_GUI_FAN_DIR, fileName);
            }
