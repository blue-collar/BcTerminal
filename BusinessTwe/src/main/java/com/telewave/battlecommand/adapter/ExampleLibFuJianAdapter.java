 ...
 * @date 2019/1/28
 */
public class ExampleLibFuJianAdapter extends BaseAdapter {
    private Context mContext;
    private List<ExampleBean.FileListBean> fileListBeanList;

    public ExampleLibFuJianAdapter(Context mContext, List<ExampleBean.FileListBean> fileListBeanList) {
        super();
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        final ExampleBean.FileListBean fileListBean = fileListBeanList.get(position);
        holder.fujianFileName.setText(fileListBean.getFilename());
        holder.fujianUpdateTime.setText(fileListBean.getUpdatetime());

        File file = new File(ConstData.EXAMPLE_LIB_DIR + fileListBean.getFilename());
        if (file.exists()) {
    ...
    class OnClickTextListener implements View.OnClickListener {
        ...
        @Override
        public void onClick(View v) {
            File file = new File(ConstData.EXAMPLE_LIB_DIR + fileName);
            if (file.exists()) {
            ...
            if (TextUtils.isEmpty(HTTP_URL)) {
                //从服务器下载预案文档
                downLoadFile(mContext, viewHolder, ConstData.urlManager.baseFilesURL + downloadUrl, ConstData.EXAMPLE_LIB_DIR, fileName);
            } else {
                //从服务器下载预案文档
                downLoadFile(mContext, viewHolder, ConstData.urlManager.baseFilesURL + HTTP_URL, ConstData.EXAMPLE_LIB_DIR, fileName);
            }
        }
