...
public class ImportantUnitInfoPresenter extends BasePresenter<IDirectionContract.IUnitView>
        implements IDirectionContract.ImportUnitInfoPresenter {

    private static final String TAG = ImportantUnitInfoPresenter.class.getSimpleName();
    ...
    public ImportantUnitInfoPresenter(IDirectionContract.IUnitView view) {
        super(view);
    }
    ...
    @Override
    public void getImportUnitInfo(String organId, String name, int currentPage, int pageSize, boolean isShowProgress) {
        ...
        final Request<String> request = NoHttp.createStringRequest(ConstData.urlManager.getImportunitList, RequestMethod.GET);

        request.add("objname", name);
        request.add("officeId", organId);
        request.add("pageNo", currentPage);
        request.add("pageSize", pageSize);
        request.setCacheKey(ConstData.KNOWLEDGE_IMPORTANT_CACHEKDY);
                    ...
                    public void onCompleted() {
                        ...
                        if (isViewAttached()) {
                            ...
                            if (unitInfoList.size() == 0) {
                                ...
                                mViewRef.get().onUnitListCompleted(unitInfoList);
                            }
                        }
                    }
