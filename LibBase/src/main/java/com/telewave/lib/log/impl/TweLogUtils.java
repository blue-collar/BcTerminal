...
public class TweLogUtils {
    ...
    public static File getLogFileDirectory(String packageName, boolean dataStorage) {
        String logPtah;
        if ("android".equals(packageName)) {    //rick_Note 这里需要check一下是否存在sys的 twe
            logPtah = "data/system/twe" + "/log/";
    ...
    public static File getCrashFileDirectory(String packageName) {
        String path;
        if ("android".equals(packageName)) {    //rick_Note 这里需要check一下是否存在sys的 twe应用
            path = "data/system/twe" + "/crash/";
    ...
    public interface ProcessHandler {
        Object handleProcessInputStream(InputStream in);
    }
    ...
    private static String getLogCfgPath(String packageName) {
        String logFile = "";
        if ("android".equals(packageName)) {    //rick_Note 这里需要check一下是否存在sys的 twe应用
            logFile = "data/system/twe" + LOG_CFG_DIR + File.separator + LOG_CFG_FILENAME;
        } else if (isExternalStorageAvailable()) {
            String logCfgDir = Environment.getExternalStorageDirectory().getAbsolutePath() + "/twe/fireepass" + LOG_CFG_DIR;
            logFile = logCfgDir + File.separator + LOG_CFG_FILENAME;
            Log.d("==========", "logCfgDir=" + logCfgDir);
        }
    ...
    private static int getNetType(Context context) {
        ...
        try {
            ...
            TweLog.w("TweLogUtils", "getNetType-> e:" + e + ", err msg: " + e.getMessage());
        }
        return TYPE_UNKNOWN;
    }
}
