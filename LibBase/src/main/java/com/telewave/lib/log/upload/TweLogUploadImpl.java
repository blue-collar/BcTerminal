...
import com.telewave.lib.log.TweLog.LOG_REPORT_ERRCODE;
import com.telewave.lib.log.TweLog.REPORT_NET_TYPE;
import com.telewave.lib.log.impl.TweLogContants;
import com.telewave.lib.log.impl.TweLogImpl;
import com.telewave.lib.log.impl.TweLogReceiverImpl;
import com.telewave.lib.log.impl.TweLogUtils;
import com.telewave.lib.log.upload.UploadLogTask.ILogTransferStatusListener;
 ...
 */
public class TweLogUploadImpl implements Callback, ILogTransferStatusListener {

    private String TAG = "TweLogUploadImpl";


    /**
     * 上报日志相关信息
     */
    public final static String ACTION_REPORT_LOG_INFO = ".twe.intent.action.REPORT_LOG_INFO";
    /**
     * 上报日志相关信息
     */
    public final static String ACTION_WUP_LOGSDK_GETTICKET_INFO = ".twe.intent.action.wup.logsdk.getLogTicket";

    private String M_ACTION_REPORT_TICKET = null;

    private static TweLogUploadImpl mInstance;
    ...
    private TweLogUploadImpl() {
        M_PID = android.os.Process.myPid();
    }

    public static TweLogUploadImpl getInstance() {

        if (mInstance == null) {
            mInstance = new TweLogUploadImpl();
        }
        return mInstance;
    }
    ...
    private void init() {
        if (mHandler == null) {
            mHandler = new Handler(TweLogImpl.getInstance().getLogThreadLooper(), this);
        }
    }
...
//        removeMsg(MSG_RECEIVE_TICKET);
    ...
    public synchronized int sendReportLogInfoMsg(AppBussInfo appBussInfo) {
        Context context = TweLogImpl.getInstance().getContext();
        if (context == null || appBussInfo == null) {  // context 为空，未注册对应广播消息（无法获取tickt，不上报）
            TweLog.w(TAG, "sendReportLogInfoMsg-> context is null or appBussInfo is null, cancel!");
            return LOG_REPORT_ERRCODE.ERR_NOT_REGISTE;
        }

        if (!TweLogImpl.getInstance().getDebugMode()) {  // relese版本，不主动上传日志
            if (appBussInfo.mReportType != LOG_REPORT_TYPE_TOOL
                    || !TweLogReceiverImpl.PERMISSION_PACKAGE.equals(
                    TweLogImpl.getInstance().getPkgName())) {  // 非小工具不允许上报上报
                TweLog.w(TAG, "sendReportLogInfoMsg-> release app but not debugTool, not report!");
                return LOG_REPORT_ERRCODE.ERR_APP_ILLEGAL;
            }
        }

        if (!isNeedSendReq()) {  // 请求太频繁或其他异常，取消请求
            TweLog.w(TAG, "sendReportLogInfoMsg-> isNeedSendReq : false, cancel!");
            return LOG_REPORT_ERRCODE.ERR_REQUEST_FREQ;
        }

        if (!isNetTypeOk(appBussInfo.mNetType, context)) {
            TweLog.w(TAG, "sendReportLogInfoMsg-> isNetTypeOk : false, cancel!");
    ...
    private boolean isNetTypeOk(int netType, Context context) {
        ...
        if (netType == REPORT_NET_TYPE.REPORT_NET_WIFI
                && TweLogUtils.isWifiMode(context)) {  // 当前是wifi状态
            res = true;
        } else {
            res = netType == REPORT_NET_TYPE.REPORT_NET_ALL;
        }
        TweLog.v(TAG, "isNetTypeOk-> res = " + res + ", netType =" + netType);
        return res;
    }
    ...
    private synchronized int onProcessLogTicketInfoMsg(int resId, AppBussInfo appBussInfo) {
        ...
        if (context == null || appBussInfo == null) {  // context 为空
            TweLog.w(TAG, "onProcessLogTicketInfoMsg-> context or appBussInfo is null, cancel! resId = " + resId);
            return -11;
        }
        ...
        try {
            ...
            if (!isNeedSendReq()) {
                TweLog.w(TAG, "onProcessLogTicketInfoMsg-> isNeedSendReq : false, cancel! resId = " + resId);
                return -13;
            }

            if (!isNeedStartUpload()) {  // 不能启动新的任务了
                TweLog.w(TAG, "onProcessLogTicketInfoMsg-> MAX_RUN_TASK_CNT, delay resId = " + resId);
                mAppBussCache.put(resId, appBussInfo);
                return -14;
            }

            if (!isNetTypeOk(appBussInfo.mNetType, TweLogImpl.getInstance().getContext())) {
                TweLog.w(TAG, "onProcessLogTicketInfoMsg-> isNetTypeOk : false, cancel! resId = " + resId);
                return -15;
            }

            String pkgName = TweLogImpl.getInstance().getPkgName();
            ...
            // 必填数据
            intent.putExtra(TweLogContants.PARAM_KEY_APP_PKGNAME, pkgName);
            intent.putExtra(TweLogContants.PARAM_KEY_TICKET_TIMEOUT, CHECK_LOG_TICKET_DELAY);
            intent.putExtra(TweLogContants.PARAM_KEY_REPORT_RESID, resId);
            intent.putExtra(TweLogContants.PARAM_KEY_REPORT_PID, M_PID);
//            // 若有其他额外附加数据，通过bundle 传递, wup sdk则将该数据返回
//            intent.putExtra(TweLogContants.PARAM_KEY_REPORT_EXTRA_DATA, null);
            ...
            TweLog.w(TAG, "onProcessLogTicketInfoMsg-> e: " + e + ", err msg = " + e.getMessage());
        }
        TweLog.d(TAG, "onProcessLogTicketInfoMsg-> resId = " + resId);
        return resId;
    }
    ...
    public String getReportTicktAction(Context context) {
        ...
        if (M_ACTION_REPORT_TICKET == null) {
            ...
            M_ACTION_REPORT_TICKET = pkgName + TweLogUploadImpl.ACTION_REPORT_LOG_INFO;
        }
    ...
    private boolean isNeedStartUpload() {
        if (mRunningTaskCache.size() >= MAX_RUN_TASK_CNT) {
            TweLog.w(TAG, "isNeedStartUpload-> MAX_RUN_TASK_CNT! ");
            return false;
        }
        return true;
    }
    ...
    private void reportLogInfo(int resId, final AppBussInfo appBussInfo) {
        ...
        if (appBussInfo == null || resId <= 0) {
            TweLog.w(TAG, "reportLogInfo-> param is err, resid = " + resId);
            return;
        }

        if (mAppBaseInfo == null || mAppBaseInfo.mTicket == null) {
            TweLog.w(TAG, "reportLogInfo-> mAppBaseInfo is err!");
            return;
        }

        if (!isNeedSendReq()) {
            TweLog.w(TAG, "reportLogInfo-> isNeedSendReq : false, cancel! resid = " + resId);
            return;
        }

        if (!isNetTypeOk(appBussInfo.mNetType, TweLogImpl.getInstance().getContext())) {
            TweLog.w(TAG, "reportLogInfo-> isNetTypeOk : false, cancel! resid = " + resId);
            return;
        }
        ...
        if (!isNeedStartUpload()) {  // 不能启动新的上传任务，已打到最大限制
            TweLog.w(TAG, "reportLogInfo-> MAX_RUN_TASK_CNT! delay resId = " + resId);
            mAppBussCache.put(resId, appBussInfo);
            return;
        }
        ...
        TweLog.v(TAG, "reportLogInfo-> mRunningTaskCache.put task resId = " + resId + ", buss resId = " + appBussInfo.mResId);
    ...
    private void onCheckRestartUploadInfo() {
        if (mAppBussCache.size() == 0) {
            TweLog.d(TAG, "checkUploadInfo-> no data need upload, cancel");
            return;
        }
    ...
    private void onCancelRequest(int resId) {
        TweLog.d(TAG, "onCancelRequest-> resId: " + resId);
        AppBussInfo appBussInfo = null;

        UploadLogTask uploadLogTask = null;
        if (resId < 0) {  // 取消所有上传信息
            TweLog.d(TAG, "onCancelRequest-> cancel all request!");
            mAppBussCache.clear();
    ...
    private void onProcessTicketFailed(int resId, int rspCode) {
        ...
        TweLog.w(TAG, "handleMessage->MSG_TICKET_FAIL: resId = " + resId + ", rspCode: " + rspCode
    ...
    private boolean isNeedSendReq() {
        ...
        return (subTime <= 0 || subTime >= mUploadDelay)
                && (subTime >= 0 || subTime <= -mUploadDelay);
    }

    @Override
    public boolean handleMessage(Message msg) {
        int what = msg.arg1;
        TweLog.i(TAG, "handleMessage-> what = " + what);
        int resId = 0;
        AppBussInfo appBussInfo = null;
        try {
            switch (what) {
                case MSG_SEND_REQUEST:  // 收到log上报的请求
                    ...
                    TweLog.d(TAG, "handleMessage-> MSG_SEND_REQUEST, rspId = " + resId
                            + ", result = " + result);
                    break;
            ...
            TweLog.w(TAG, "handleMessage-> e:" + e + ", err msg: " + e.getMessage());
        }
    ...
    public void onLogTransferStarted(AppBussInfo appBussInfo) {
        TweLog.i(TAG, "onLogTransferStarted -> resId = " + appBussInfo.mResId);
    }

    @Override
    public void onLogTransferEnd(AppBussInfo appBussInfo) {
        TweLog.i(TAG, "onLogTransferEnd -> resId = " + appBussInfo.mResId);
    ...
    public boolean isBussInfoValid(AppBussInfo appBussInfo) {
        ...
        if ((tempInfo != null && tempInfo.mRunState < 0)
            ...
            TweLog.w(TAG, "isBussInfoValid-> resId = " + resId + ", task: " + uploadLogTask);
            return false;
        }
        return true;
    }
        ...
        public boolean handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case 0:
                    ...
                    if (pkgName == null || "".equals(pkgName)) {
                        pkgName = TweLogImpl.getInstance().getPkgName();
                    }
                    TweLog.d(TAG, "reportLogInfo-> uploadLogTask : start resId = " + msg.arg1
                            + ", pkg name = " + pkgName);
