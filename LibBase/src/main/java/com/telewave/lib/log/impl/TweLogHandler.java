 ...
 */
public class TweLogHandler extends Handler {
    ...
    public TweLogHandler(Looper looper, TweLogBaseConfig config) {
        super(looper);
        mConfig = config;
    ...
    private String getLogFileSubName(boolean isTrace) {
        if (mProcessName == null) {
            mProcessName = TweLogUtils.getProcessName();
    ...
    public ArrayList<File> prepareUploadTraceLog() {
        ...
        File dir1 = TweLogUtils.getLogFileDirectory(mConfig.getPackageName(), true);
        File dir2 = TweLogUtils.getLogFileDirectory(mConfig.getPackageName(), false);
    ...
    private void resetCurFileLog() {
        ...
        if (mLogFile != null) {
            ...
            File dir = TweLogUtils.getLogFileDirectory(mConfig.getPackageName(), false);
        ...
        switch (msg.what) {
            case LOG_MSG_LOG_WRITE:  // debug的文件日志
                ...
                try {
                    if (!mLogWriterFlag || mLogFileDate != mCalendar.get(Calendar.DAY_OF_MONTH)) {
                        File dir = TweLogUtils.getLogFileDirectory(mConfig.getPackageName(), false);
                        if (dir != null) {
                            mLogFile = TweLogUtils.createNewFile(dir.getAbsolutePath(), "log_" + getLogFileSubName(false) + ".log");
                    ...
                    if (ex.getMessage() != null)
                        TweLog.w("TraceLog", ex.getMessage());
                    else
                        TweLog.w("TraceLog", "Write log catch a null exception!!!");
                }
                break;
            ...
            case LOG_MSG_TRACE_WRITE:  // 用户行为的trace日志
                ...
                if (!mTraceWriterFlag || mTraceFileDate != mCalendar.get(Calendar.DAY_OF_MONTH)) {
                    File dir = TweLogUtils.getLogFileDirectory(mConfig.getPackageName(), false);
                    if (dir != null) {
                        mTraceFile = TweLogUtils.createNewFile(dir.getAbsolutePath(), "trace_" + getLogFileSubName(true) + ".txt");
                        if (mTraceFile != null) {
            ...
            case LOG_MSG_FILE_CLEAN:  // 清理过期日志
                delExpiredLogFile(TweLogUtils.getLogFileDirectory(mConfig.getPackageName(), true), 3 * LOG_DAYMILLISECOND);
                delExpiredLogFile(TweLogUtils.getLogFileDirectory(mConfig.getPackageName(), false), 3 * LOG_DAYMILLISECOND);
                delExpiredLogFile(TweLogUtils.getCrashFileDirectory(mConfig.getPackageName()), 3 * LOG_DAYMILLISECOND);
                break;
